...
// Load blocklist and lock timer
chrome.storage.local.get(["blocklist", "lockUntil"], (data) => {
...
saveBtn.addEventListener("click", () => {
  const lines = area.value
    ...
    .map((s) => s.trim())
    .filter(Boolean);
  chrome.runtime.sendMessage({ type: "SET_BLOCKLIST", blocklist: lines }, () =>
...
startTimerBtn.addEventListener("click", () => {
  ...
  if (minutes > 0) {
    ...
    chrome.runtime.sendMessage({ type: "SET_STUDY_MODE", enabled: true });
    toggle.checked = true;
